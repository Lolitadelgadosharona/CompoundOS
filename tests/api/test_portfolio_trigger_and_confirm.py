...
def test_alembic_revision_chain_valid(postgres_engine) -> None:
    ...
    assert heads[0].revision == "0019_investment_policy_enrichment", (
        f"Expected head 0019_investment_policy_enrichment, "
...
def test_migration_chain_0004_0005_0006(postgres_engine) -> None:
    ...
    with postgres_engine.connect() as conn:
        ...
        assert row[0] == "0019_investment_policy_enrichment"
        assert len(row[0]) <= 32
