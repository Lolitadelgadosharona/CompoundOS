...
def test_alembic_revision_chain_valid(postgres_engine) -> None:
    ...
    assert heads[0].revision == "0017_schedule_allowlist_backup_daily", (
        f"Expected head 0017_schedule_allowlist_backup_daily, "
...
def test_migration_chain_0004_0005_0006(postgres_engine) -> None:
    ...
    with postgres_engine.connect() as conn:
        ...
        assert row[0] == "0017_schedule_allowlist_backup_daily"
        assert len(row[0]) <= 32
