...
def test_alembic_revision_chain_valid(postgres_engine) -> None:
    ...
    assert heads[0].revision == "0032_decision_lifecycle_hardening", (
        f"Expected head 0032_decision_lifecycle_hardening, "
...
def test_migration_chain_0004_0005_0006(postgres_engine) -> None:
    ...
    with postgres_engine.connect() as conn:
        ...
        assert row[0] == "0032_decision_lifecycle_hardening"
        assert len(row[0]) <= 32
